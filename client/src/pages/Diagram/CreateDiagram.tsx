...
import {
  ...
  Save, // Added for history sidebar toggle
} from "lucide-react";
...
const MermaidEditor = () => {
  ...
  // [CORE] function to save the diagram
  const handleSaveRequest = async () => {
    /*
    The logic shoube be like this:

    if(id is present){
        If present, try to request the backend to update the diagram.
        The backend will check if the user is the owner or has edit access or not.
        Dont send the access seetings from the frontend. Just send the user token. 
        If the user has the access, update the diagram, else return error.
    } else{
        If not present, create a new diagram.
    }
     */
    ...
    navigate(`/diagram/${diagram.mode}/${diagram._id}/`);
  };

  useEffect(() => {
    if (id && access) {
      // ensure id and access are present
  ...
  // [CORE] function to handle history item click
    ...
    <div className="flex gap-4 p-4 items-start relative">
      ...
      <div>
        <div className="rounded-lg bg-slate-100 p-2 flex flex-col items-center my-5">
          ...
          <button
            className={`bg-black text-white px-4 py-2 rounded font-extrabold m-0.5 my-1 ${
              privellege === "owner" || privellege === "edit" || !access
                ? ""
          ...
          <button
            className={` text-white px-4 py-2 rounded font-extrabold m-0.5 my-1 bg-black ${
              id && owner === user?.email ? "" : "opacity-50 cursor-not-allowed"
            }`}
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg relative w-full max-w-md">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="rounded-lg overflow-auto w-full flex  items-center ">
                <label
                  htmlFor="exportName"
                  className="pr-2 font-black text-sm whitespace-nowrap"
                >
                  Name
                </label>
                <input
